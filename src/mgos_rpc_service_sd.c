...
static void rpc_sd_open(struct mg_rpc_request_info *ri, void *cb_arg,
  ...
  if (NULL != sd) {
    //error
    mg_rpc_send_errorf(ri, 400, "SD already open as %s!", mgos_sd_get_mount_point());
    ri = NULL;
    return;
  }
  ...
  if (NULL == sd) {
    mg_rpc_send_errorf(ri, 400, "Failed to open SD (args: %.*s)",
                       args.len, args.p);
    return;
  }
...
static void rpc_sd_get_mount_point(struct mg_rpc_request_info *ri,
